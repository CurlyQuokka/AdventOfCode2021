...
func HexRune2Bin(hex rune) (string, error) {
	var err error
	if parsed, err := strconv.ParseUint(string(hex), 16, 64); err == nil {
...
func HexStringToBin(hex *string) string {
	binary := ""
	for _, c := range *hex {
		b, err := HexRune2Bin(c)
		if err != nil {
...
func BinStringToDec(bin string) uint64 {
	var parsed uint64
	var err error
	if parsed, err = strconv.ParseUint(bin, 2, 64); err != nil {
		fmt.Println(err.Error())
		os.Exit(465)
	}
	return parsed
